...
@section('content')
    ...
    <div class="detail-wrap">
        <form method="POST" action="/attendance/detail/{{ $attendance->id }}">
            ...
            {{-- 承認待ちメッセージ --}}
            @if($isPending)
                <div class="pending-msg">承認待ちのため修正はできません。</div>
            @endif
            ...
            <div class="row">
                ...
                <div class="value time-fields">
                    <input type="text" name="clock_in"
                        ...
                        placeholder="00:00" {{ $isPending ? 'disabled' : '' }}>
            ...
            <div class="row">
                ...
                <div class="value time-fields">
                    <input type="text" name="breaks[{{ $attendance->breakTimes->count() }}][start]"
                        placeholder="00:00" {{ $isPending ? 'disabled' : '' }}>
                    <span class="tilde">〜</span>
                    <input type="text" name="breaks[{{ $attendance->breakTimes->count() }}][end]"
                        placeholder="00:00" {{ $isPending ? 'disabled' : '' }}>
                </div>
            </div>
            ...
            <div class="row">
                ...
                <div class="value">
                    <textarea name="note" rows="3"
                        {{ $isPending ? 'disabled' : '' }}>{{ $attendance->note }}</textarea>
                </div>
            </div>
